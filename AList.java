...
class AList<T> implements ListInterface<T> {
    ...
    */
    public void reverse()
    {
    	//Initializing start variable to position 1
    	int start= 1;  
    	//Initializing end variable to maximum number of entries
    	int end = numberOfEntries;
    	//swapping the values by storing in a temporary node
    	while (start < end)
        {
            T temp = list[start]; 
           list[start] = list[end];
   ...
   public void cycle()
    {
	   //saving the first node in a temporary node	  
	   T temp = list[1]; 
	   //initialising Loop frm 2 and shifting the values Backwards
	   for (int i = 2; i <= numberOfEntries; i++)
	   {  
		   list[i-1] = list[i];
		   
	   }
     //Storing the first value in the last position 
